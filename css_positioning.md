...
Now we'll set paragraph one to have a width of 50vw.
...
```
Notice the small difference in widths? The first paragraph is taking up half of the viewport screen and the second paragraph is taking up half of the width of the parent div. To make this more evident, we can resize the div using a fixed amount of pixels.
```css
#paragraph-two {
  border: 2px solid red;
  width: 600px;
...
Let's change the margin's on each to be 0em;
...
Notice that each box still has its own padding. 1ems of padding on the top and bottom of each box create a total of 2em's of padding.
...
To illustrate this let's get rid of the all of the existing css for paragraphs two and three and set them to have different borders and padding of 1em on all sides.
...
<!-- DELETE THE PARAGRAPH ONE AND TWO CSS IF THAT IS DECIDED  -->
...
The item being set to float does not have to be a smaller element inside of another. Let's get rid of the floater and set the fifth paragraph to float left instead.
...
Let's remove those last few lines...
...
Let's select our list elements at the top of the page and set them to inline which does exactly what it sounds like.
...
When it come to laying elements out onto a page, perhaps the most powerful CSS style is position.​ Wrestling control of element positioning from the browser can be tough, but it’s the only way to have full control.  
...
Now we can see that the title has shifted down 2em and left 2em from its original position.
